        ...
        ctx.get().enqueueWork(() -> {
            ...
            if(packet.hand == 0) {
                if (((Player) world.getPlayerByUUID(packet.entityId)).getMainHandItem().getItem() == packet.flute.getItem())
                    ((Player) world.getPlayerByUUID(packet.entityId)).getMainHandItem().getOrCreateTag().putInt("helpCommandSelected", packet.helpCommand);
            }
            else
            {
                if (((Player) world.getPlayerByUUID(packet.entityId)).getOffhandItem().getItem() == packet.flute.getItem())
                    ((Player) world.getPlayerByUUID(packet.entityId)).getOffhandItem().getOrCreateTag().putInt("helpCommandSelected", packet.helpCommand);
            }
        });
